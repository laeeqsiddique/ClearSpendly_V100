-- Polar Integration Migration for ClearSpendly - FINAL CORRECTED VERSION
-- This version handles all PostgreSQL constraint syntax properly
...
-- 3. Add Polar-specific fields to subscription table
DO $$
BEGIN
...
-- Add unique constraint for Polar subscription ID
DO $$
BEGIN
...
-- Add unique constraint for billing history
DO $$
BEGIN
...
-- Add unique constraint for usage quota
DO $$
BEGIN
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view their tenant subscription events" ON public.subscription_event;
  DROP POLICY IF EXISTS "Users can view their tenant billing history" ON public.subscription_billing_history;
  DROP POLICY IF EXISTS "Users can view their tenant usage quotas" ON public.usage_quota;
  DROP POLICY IF EXISTS "Admins can manage their tenant usage quotas" ON public.usage_quota;
END $$;
...
-- 17. Insert sample Polar configuration for testing (only if not exists)
...
-- Success message
SELECT 'Polar integration migration completed successfully!' as result;