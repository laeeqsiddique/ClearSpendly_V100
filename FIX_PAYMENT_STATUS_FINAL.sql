-- ===================================================================
-- FIX: Payment Status Bug - Final Corrected Version
-- ===================================================================
...
BEGIN
    ...
    IF v_total_paid = 0 THEN
        ...
        SELECT status INTO v_new_invoice_status FROM public.invoice WHERE id = v_invoice_id;
    ...
    -- Update invoice with calculated values (EXCLUDE generated columns)
...
BEGIN
    ...
    LOOP
        -- Update each invoice with correct payment status (EXCLUDE generated columns)
        UPDATE public.invoice
        SET 
            ...
            status = CASE
                WHEN invoice_record.total_paid >= invoice_record.total_amount THEN 'paid'
                ELSE invoice_record.status  -- Keep existing status for unpaid/partial
            END,
            updated_at = CURRENT_TIMESTAMP
...
-- Show current payment issues (for debugging) - FIXED QUERY
WITH invoice_payments AS (
    SELECT 
        ...
        i.payment_status,
        i.status,
        COALESCE(SUM(pa.allocated_amount), 0) as calculated_paid
    FROM public.invoice i
    LEFT JOIN public.payment_allocation pa ON i.id = pa.invoice_id
    GROUP BY i.id, i.invoice_number, i.total_amount, i.amount_paid, i.balance_due, i.payment_status, i.status
)
SELECT 
    invoice_number,
    total_amount,
    ...
    calculated_paid,
    CASE 
        WHEN amount_paid != calculated_paid THEN 'AMOUNT_MISMATCH'
        WHEN amount_paid > 0 AND payment_status = 'unpaid' THEN 'STATUS_WRONG'
        WHEN amount_paid = total_amount AND payment_status != 'paid' THEN 'SHOULD_BE_PAID'
        ELSE 'OK'
    END as issue_type
FROM invoice_payments
WHERE amount_paid != calculated_paid
   OR (amount_paid > 0 AND payment_status = 'unpaid')
   OR (amount_paid = total_amount AND payment_status != 'paid')
