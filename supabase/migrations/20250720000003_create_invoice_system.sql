...
CREATE TABLE IF NOT EXISTS public.client (
  ...
  -- Business details
  tax_id VARCHAR(50), -- Client's tax ID if needed
  notes TEXT,
  
  -- Payment preferences
  payment_terms VARCHAR(50) DEFAULT 'Net 30', -- Net 15, Net 30, Due on Receipt, etc.
  preferred_payment_method VARCHAR(50) DEFAULT 'stripe', -- stripe, bank_transfer, check
...
CREATE TABLE IF NOT EXISTS public.invoice_template (
  ...
  name VARCHAR(100) NOT NULL, -- e.g., "Web Development", "Consulting", "Design"
  
  -- Template styling
  template_type VARCHAR(50) DEFAULT 'modern', -- modern, classic, minimal, professional
  color_scheme VARCHAR(50) DEFAULT 'blue', -- blue, green, purple, gray
  ...
  invoice_prefix VARCHAR(10) DEFAULT 'INV', -- INV, WEB, CONS, etc.
...
CREATE TABLE IF NOT EXISTS public.invoice_payment (
  ...
  payment_method VARCHAR(50) NOT NULL, -- stripe, bank_transfer, cash, check
...
CREATE TABLE IF NOT EXISTS public.invoice_activity (
  ...
  activity_type VARCHAR(50) NOT NULL, -- created, sent, viewed, paid, reminded, cancelled
  description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.recurring_invoice (
  ...
  interval_count INTEGER DEFAULT 1, -- every X weeks/months
...
-- RLS Policies for invoice activities
...
-- RLS Policies for recurring invoice items
