-- Create expense subscriptions table for tracking recurring services
...
-- Function to calculate next charge date based on frequency
...
BEGIN
  ...
  IF NEW.next_charge_date IS NULL AND NEW.start_date IS NOT NULL THEN
    NEW.next_charge_date = calculate_next_charge_date(NEW.start_date, NEW.frequency);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER expense_subscription_updated_at_trigger
  BEFORE UPDATE ON public.expense_subscription
  FOR EACH ROW
  EXECUTE FUNCTION update_expense_subscription_updated_at();

-- Add comment for documentation
COMMENT ON TABLE public.expense_subscription IS 'Tracks recurring subscription services separate from one-time expenses';